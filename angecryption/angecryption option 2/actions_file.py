...
def decrypt_to_png(msg, iv, key):
    """
    decrypt a angecrypted file back to a PNG file
    """
    res_of_decrypt = decrypt(msg, iv, key)
...
def decrypt_to_pdf(msg, iv, key):
    """
    decrypt a angecrypted file back to a PDF file
    """
    res_of_decrypt = decrypt(msg, iv, key)
...
def hide_png_in_pdf(src, tar, out, key, iv):
    ...
    encrypt and hiding the png file inside the pdf file.
    ...
    pdf = decrypt(pdf, iv, key)
    pdf += img
    pdf += b"\x00" * (BLOCK_SIZE_16 - len(pdf) % BLOCK_SIZE_16) * (len(pdf) % BLOCK_SIZE_16 != 0)
    with open(out, "wb") as f:
        f.write(encrypt(pdf, iv, key))


def hide_pdf_in_png(src, tar, out, key, iv):
    """
    taking 3 files - (png), (pdf), (png).
    encrypt and hiding the pdf file inside the png file.
